...
void checkDawn() {
  ...
  // проверка рассвета
  if (alarm1[thisDay].state &&                                       // день будильника
      thisTime >= (alarm1[thisDay].time - dawnOffsets[dawnMode]) &&  // позже начала
      thisTime < (alarm1[thisDay].time + DAWN_TIMEOUT) ) {           // раньше конца + минута
    if (!manualOff) {
      // величина рассвета 0-255
      int dawnPosition = (float)255 * ((float)((float)thisTime - (alarm1[thisDay].time - dawnOffsets[dawnMode])) / dawnOffsets[dawnMode]);
