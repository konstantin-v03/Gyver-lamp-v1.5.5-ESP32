void parseUDP() {
  ...
  if (packetSize) {
    ...
    const uint8_t* reply1 = (uint8_t*)reply;
    Udp.write(reply1, inputBuffer.length() + 1);
    Udp.endPacket();
  }
}
...
void sendAlarms() {
  inputBuffer = "ALMS ";
  for (byte i = 0; i < 7; i++) {
    inputBuffer += String(alarm1[i].state);
    inputBuffer += " ";
  }
  for (byte i = 0; i < 7; i++) {
    inputBuffer += String(alarm1[i].time);
    inputBuffer += " ";
  }
